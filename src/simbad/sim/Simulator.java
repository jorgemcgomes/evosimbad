 ...
 * <li>update sensors </li> <li>call agent performBehavior </li> <li>update
 * position </li> In normal operation the steps are triggered by a timer event.
 * <br> The Simulator class also provides a Background Mode with limited
 * rendering. This mode is mainly useful for batch simulation ( ie genetic
 * algorithms). See Also Simbatch class.
 *
...
public class Simulator {
    ...
    private float virtualTimeFactor = 1;
    /**
     ...
     */
    PhysicalEngine2 physicalEngine;
    /**
     ...
     *
     * @param applicationComponent - A reference to the main Application
     * container.
    ...
    private void initialize(EnvironmentDescription ed) {
        ...
        physicalEngine = new PhysicalEngine2();
    ...
    /**
     * Creates the UI that may be associated to each agent. If the agent has set
     * a Panel with setUIPanel a window is created containing the panel. Only
     * called once.
     *
     */
    ...
    public synchronized void simulateOneStep() {
        ...
        // call for environment updates
        environment.updateEnvironment(dt);
        ...
        physicalEngine.clearCollisions(agents);
        ArrayList randomOrderAgents = new ArrayList(agents);
        Collections.shuffle(randomOrderAgents);
        for (int i = 0; i < nagents; i++) {
            SimpleAgent a = ((SimpleAgent) randomOrderAgents.get(i));
            
            physicalEngine.agentAgentsCollisions(a, randomOrderAgents, true);
            a.integratesPositionChange(dt);

            physicalEngine.agentObjectCollisions(a, objects, true);
            a.integratesPositionChange(dt);
            
            if(physicalEngine.agentAgentsCollisions(a, randomOrderAgents, false) ||
                    physicalEngine.agentObjectCollisions(a, objects, false)) {
    ...
    private class SimulatorThread extends Thread {
        ...
        public void run() {
            ...
            /*
             * try { // First wait a bit so J3d is settled ? sleep(1000); }
            ...
            while (!stopped) {
                ...
                if ((count % rendererRate) == 0) {
                    ...
                    /*
                     * for (int i = 0; i < agents.size(); i++) { Object o =
                     * agents.get(i); if (o instanceof Agent) ((Agent)
                     * o).getAgentInspector().update();
                    }
                     */
                }
            }
