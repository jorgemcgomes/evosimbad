 ...
 * <br> When deriving a new robot's class from this class, one should override
 * the following methods: <ul> <li> initBehavior : called by the simulator to
 * initialize the controler.</li> <li> performBehavior : called by the simulator
 * on each simulation step.</li> </ul> * Implementation note : the agent doesnt
 * have synchronized methods. All thread refering to the agent should do
 * explicit synchronization with synchronized(agent){...}.
 *
 */
public class Agent extends SimpleAgent {
     ...
     */
    public Agent(Vector3d pos) {
        super(pos);
        // No UI by default
        this.panel = null;
        agentInspector = null;
        // physical parameters default --- epuck
        height = 0.06f;
        radius = 0.04f;
    ...
    protected void create3D() {
        ...
        // direction indicator
        float coords[] = {
            radius / 2, height / 2 + 0.005f, -radius / 2,//
            radius / 2, height / 2 + 0.005f, radius / 2,//
            radius, height / 2 + 0.005f, 0 //
        };
        float normals[] = {
            0, 1, 0,
            0, 1, 0,
    ...
    public void setFrameMeterRate(int rate) {
        frameMeter.setUpdateRate(rate);
    }

    /**
     * Add a UI panel to the agent. Typically used for displaying behavior
     * outputs. A call to this method will have for consequence the creation of
     * a dedicated window.
     *
     * @param panel
     */
